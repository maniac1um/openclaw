...
class GitTools:
    ...
    def commit_and_push(self, message: str, patterns: List[str], exclude_patterns: Optional[List[str]] = None) -> dict:
        """
        提交并推送修改，只允许 patterns 指定的文件或目录，可通过 exclude_patterns 排除不希望提交的路径
        ...
        if exclude_patterns:
            for pattern in exclude_patterns:
                full_pattern = os.path.join(self.repo_path, pattern)
                if not full_pattern.startswith(self.repo_path):
                    return {'success': False, 'error': f'Forbidden exclude pattern: {pattern}'}
                self._run_git_command(['reset', '--', pattern])
